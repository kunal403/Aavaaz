...
import fs from "node:fs"; //interact with file-folders system paths
import path from "node:path"; //construct and manipulate file system paths
import { fileURLToPath } from "node:url";

import "dotenv/config";
import openapiTS, { astToString } from "openapi-typescript";

const __dirname = path.dirname(fileURLToPath(import.meta.url)); //convert the file url to a file path and get the directory name
const OUTPUT_PATH = path.resolve(__dirname, "../src/types/chatterbox-api.d.ts"); //abolute location where the generated types will be written
